#!/usr/bin/env bash
# =============================================================================
# Pre-compute local PCA cache for every CelebA-HQ training image.
# Runs kNN lookup + SVD once per image, saves mean/evals/evecs to .npz.
# Must be run BEFORE submit_smooth_classifiers_celebahq.sh (manifold mode).
# Re-runnable: already-cached images are skipped automatically.
#
# Usage:
#   ./server_scripts/submit_precompute_pca_cache_celebahq.sh [--knn-k K] [--dry-run]
#
# Output:
#   output/pca_cache/celebahq/knn500_data512x512_pca_cache.npz
# =============================================================================
set -euo pipefail
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo "  ./server_scripts/submit_smooth_classifiers_celebahq.sh"
